 ...
 * check that the working shown reproduces the number shown — a line of algebra
 * with a factor missing from it is exactly the bug this text exists to prevent.
 */
...
export function readWorkings(text: string): number {
  ...
  return (base + flat) * (1 + increased / 100) * product;
}
