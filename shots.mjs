...
for (const vp of VIEWPORTS) {
  ...
  await page.evaluate(() => {
    ...
    // by choosing rather than by clicking a currency. ONE LINE A SOCKET: the
    // list is whatever socket is picked, and clicking the enabled row over and
    // over instead RAISES one socket until it fractures and the piece is bare.
    const click = (el) => el?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    for (let at = 0; at < 4; at++) {
      click(document.getElementById(`bench-socket-${at}`));
